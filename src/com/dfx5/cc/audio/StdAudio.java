    ...
    public static AudioInputStream getAudioInputStreamFromFile(String filename) {
        ...
        try {
            ...
            if (file.exists()) {
                ...
                return AudioSystem.getAudioInputStream(format,AudioSystem.getAudioInputStream(file));
...
//        AudioFormat audioFormat = new AudioFormat(SAMPLE_RATE, 16, MONO, SIGNED, LITTLE_ENDIAN);

        System.out.println("audioFormat" + audioFormat.toString());

        System.out.println("ais audioFormat" + ais.getFormat().toString());
    ...
    public static void save(String filename, double[] samples) {
        ...
        byte[] data = new byte[4 * samples.length];
        for (int i = 0; i < samples.length; i++) {
            int temp = (short) (samples[i] * MAX_16_BIT);
            data[4*i + 0] = (byte) temp;
            data[4*i + 1] = (byte) (temp >> 8);   // little endian
        }
